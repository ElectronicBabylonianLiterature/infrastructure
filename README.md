...
![image.png](/.eraser/aGds6s7Uqq35ZqoSOch1___8rEEI9hk1jV4GniI7pAK1HIhMOW2___OciNOPNVWFWa4FmsbCq3b.png "image.png")
...
- Configure the firewall. (Published ports are opened automatically by Docker with iptables and not appear in ufw rules.)
  - Allow connections from all other nodes:sudo ufw allow proto tcp from <node IP> to any port 2377,7946 comment 'Docker Swarm'
sudo ufw allow proto udp from <node IP> to any port 7946,4789 comment 'Docker Swarm'
  - Allow metrics:sudo ufw allow from 172.18.0.0/16 to any port 9323 comment 'Docker Metrics'
...
- On a manager node install [Swarmpit](https://swarmpit.io/)  with default options: Swarmpit is now accessible at port 888.docker run -it --rm \
...
- Create stack `ebl-mongodb`  from [mongodb.yml](https://github.com/ElectronicBabylonianLiterature/infrastructure/blob/master/mongodb.yml) . Initdb functionality does not work well with SSL, so we enable it in the next step. See: [https://github.com/docker-library/mongo/issues/239](https://github.com/docker-library/mongo/issues/239)  and [https://github.com/docker-library/mongo/issues/172](https://github.com/docker-library/mongo/issues/172) .
...
- Initiate the replica set. Login to mongo and run (The hosts must have full address, otherwise it is not possible to connect to the replica set from outside the stack):rs.initiate( {
   _id : "rs-ebl1",
   members: [
...
- Create a user for the exporter.db.getSiblingDB("admin").createUser({
...
- Update the `swarmprom`  stack:
  - Add `mongodb-exporter`  service: mongodb-exporter:
...
deploy:
  resources:
    ...
    limits:
      memory: 128M
  - Add `mongodb-exporter`  job to `prometheus` :JOBS: traefik:8080 mongodb-exporter:9104
...
- Update the `swarmprom`  stack:
  - Add `redis-exporter`  service: redis-exporter:
...
- net
- monitoring
  - Add `redis-exporter`  job to `prometheus` :JOBS: traefik:8080 mongodb-exporter:9216 redis-exporter:9121
...
<a href="/README-Electronic Babylonian Library Infrastructure-1.eraserdiagram" data-element-id="UVNfbY_g6XF6ZtcWBPJcl"><img src="/.eraser/aGds6s7Uqq35ZqoSOch1___8rEEI9hk1jV4GniI7pAK1HIhMOW2___---diagram----a79e88b4a18793075918fd46bb256ce6-Electronic-Babylonian-Library-Infrastructure.png" alt="" data-element-id="UVNfbY_g6XF6ZtcWBPJcl" /></a>
